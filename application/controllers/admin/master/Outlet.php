...
class Outlet extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->render('admin/master/Outlet_view');
    }
    ...
    public function get_data_all(){
        ...
        foreach ($list as $dt) {
            ...
            $row[] = $dt->id;
            $row[] = $dt->nama_outlet;
            $row[] = $dt->alamat_outlet;
            $row[] = $dt->kota_outlet;
            $row[] = $dt->telp_outlet;
            $row[] = $dt->hp_outlet;
            $row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="edit_outlet('."'".$dt->id."'".')"><i class="glyphicon glyphicon-pencil"></i> Edit</a>
                  <a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_outlet('."'".$dt->id."'".')"><i class="glyphicon glyphicon-trash"></i> Delete</a>';
            $data[] = $row;
        }
